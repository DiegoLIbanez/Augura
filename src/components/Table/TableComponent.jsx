...
function TableComponent({ setView, dataList, ...props }) {
  ...
  <table className="w-full text-left table-auto min-w-max">
    <thead>
      <tr>
        {header.map((item, i) => (
          <th key={i} className="p-4 border-b border-slate-200 bg-gray-900">
    ...
    <tbody>
      {(currentData ?? []).length === 0 ? (
        <tr>
          <td colSpan="15" className="text-center p-4 text-sm text-slate-500">
            No hay registros
          </td>
        </tr>
      ) : (
            ...
            {columns.map((col, index) => (
              <td key={index} className="p-4 py-2">
                {col.render ? (
                  ...
                  <p className="text-sm text-slate-500">{item[col.accessor] || "N/A"}</p>
                )}
              </td>
            ))}
          </tr>
