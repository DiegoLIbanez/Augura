export function vehicleFilter( state, typeVehicleId, companyId, statusDesinfectionId, typeBurdenId, typeCommunalId, typeInputId, searchTerm, startDate, endDate ) {
  return (
    state?.filter((item) => {
      ...
      const matchesSearch =
        searchLower === "" ||
        // item.user?.[0]?.dni?.toLowerCase().includes(searchLower) ||
        // item.user?.[0]?.name?.toLowerCase().includes(searchLower) ||
