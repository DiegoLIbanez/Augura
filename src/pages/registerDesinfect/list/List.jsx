...
function List() {
  ...
  const columns = [
    ...
    { header: "Teléfono", render: (item) => item?.person?.[0]?.phoneNumber || "N/A" },
    { header: "Rol", render: (item) => item?.person?.[0]?.role?.description || "N/A" },
    { header: "Status", render: (item) => item?.person[0].status.description || "N/A" },
    { header: "Placa", render: (item) => item?.vehicle[0].plate || "N/A" },
    { header: "Tipo de Vehiculo", render: (item) => item?.vehicle[0].typeVehicle[0].description || "N/A" },
    { header: "Compañia", render: (item) => item?.vehicle[0].company[0].name || "N/A" },
    { header: "Estado de Desinfeccion", render: (item) => item?.statusDesinfection[0].description || "N/A" },
    { header: "Tipo de Carga", render: (item) => item?.typeBurden[0].description || "N/A" },
    { header: "Tipo de Comunal", render: (item) => item?.typeCommunal[0].description || "N/A" },
    { header: "Tipo de insumo", render: (item) => item?.typeInput[0].description || "N/A" },
    { header: "Destino Inicial", render: (item) => item?.initialDestination || "N/A" },
    { header: "Destino Final", render: (item) => item?.endDestination || "N/A" },
    { header: "Fecha Registro", render: (item) => fecha(item.createdAt) || "N/A" },
    { header: "Acciones", 
      render: (item) => (
          ...
          <div className="flex flex-col w-full mx-auto gap-9 bg-white shadow-md rounded-lg overflow-hidden">
            <div className="flex flex-col md:flex-row lg:w-1/2 md:w-full gap-6 p-4">
              ...
              <div className="flex flex-col flex-1">
                ...
                <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="mt-1 p-2 border rounded-md focus:ring-2 focus:ring-indigo-500"/>
              </div>
              ...
              <div className="flex flex-col flex-1">
                ...
                <input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="mt-1 p-2 border rounded-md focus:ring-2 focus:ring-indigo-500" />
              </div>

              <div className="flex flex-col justify-end">
                <button onClick={() => { setStartDate(""); setEndDate(""); }} className="h-[42px] px-4 text-sm bg-gray-200 hover:bg-gray-300 rounded-md transition-colors" >
                  Limpiar fechas
                </button>
              </div>
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-4 p-4">
              <div className="flex gap-6">
                <button onClick={() => handleExport(false)} className="h-[42px] px-4 text-sm bg-green-600 hover:bg-green-700 text-white rounded-md transition-colors" > 
                  Exportar Filtrados
                </button>
                <button onClick={() => handleExport(true)} className="h-[42px] px-4 text-sm bg-blue-600 hover:bg-blue-700 text-white rounded-md transition-colors" >
                  Exportar Todos
                </button>
              </div>
              ...
              <div className="flex gap-4">
                <div className="bg-red-100 px-3 py-1 rounded-md">
                  <span className="text-red-700 font-medium">
                    Vehiculos No Desinfectados: {countVehicleDesinfection.length}
                  </span>
                </div>
              ...
              <div className="w-full flex flex-col sm:flex-row justify-between items-center mb-3 mt-1 pl-3">
                <div className="flex flex-col sm:flex-row sm:space-x-4">
                  ...
                  <div className="flex flex-col mb-3 sm:mb-0">
                    <label htmlFor="project" className="text-sm font-medium text-slate-800 text-center"> 
                      Tipo de Vehiculo
                    </label>
                    <select value={typeVehicleinput} onChange={handleProjectChange} id="project" className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                      <option>---Todos---</option>
                  ...
                  <div className="flex flex-col mb-3 sm:mb-0">
                    <label htmlFor="company" className="text-sm font-medium text-slate-800 text-center" >
                      Compañia
                    </label>
                    <select value={companyinput} onChange={handleCompanyChange} id="company" className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md" >
                      <option>---Todos---</option>
                  ...
                  <div className="flex flex-col mb-3 sm:mb-0">
                    ...
                    <select value={statusDesinfectioninput} onChange={handleStatusDesinfectionChange} className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                      <option>---Todos---</option>
                  ...
                  <div className="flex flex-col mb-3 sm:mb-0">
                    ...
                    <select value={typeBurdeninput} onChange={handleTypeBurdenChange} className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                      <option>---Todos---</option>
                  ...
                  <div className="flex flex-col mb-3 sm:mb-0">
                    <label htmlFor="status" className="text-sm font-medium text-slate-800 text-center" >
                      Tipo de Comunal
                    </label>
                    <select value={typeCommunalinput} onChange={handleTypeCommunalChange} className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                      <option>---Todos---</option>
                  ...
                  <div className="flex flex-col mb-3 sm:mb-0">
                    ...
                    <select value={typeInputinput} onChange={handleTypeInputChange} className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                      <option>---Todos---</option>
